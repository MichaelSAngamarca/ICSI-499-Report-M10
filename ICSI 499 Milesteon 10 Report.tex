...
Offline mode in TalkAssist is designed to ensure continuous accessibility when internet connectivity is unavailable. The system continuously monitors network availability using a lightweight connection check to a reliable external endpoint. If the connection fails, TalkAssist automatically transitions into offline mode without requiring user intervention. When connectivity is restored, the system seamlessly switches back to online mode while the application remains running.

In offline mode, all processing is performed locally on the user's device. Speech to text is handled using a locally deployed whisper model, while text to speech output is generated using offline capable libraries such as pyttsx3. Core functionalities including time and date queries, reminder creation and retrieval, and basic mathematical computations remain fully available. Reminders are stored locally in a JSON file to preserve functionality without cloud dependency.

This design prioritizes reliability, privacy and user independence, allowing visually impaired users to interact with TalkAssist confidently in environments with limited or no internet access.
...
Mostly focused on the offline mode. Nearly all offline functions were completed, and the online mode's reminders functionality was also completed. Worked with other members to integrate the back end to the GUI and the online mode to the offline mode.  Helped finish the Talk Assist documentation (Milestone Videos, Slideshow, M10 Doc).
