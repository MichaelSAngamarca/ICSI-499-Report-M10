...
\noindent
Team 5 would like to extend our heartfelt appreciation to Professor Pradeep Atrey and Athulya Mathew for their unwavering support and encouragement. Their guidance throughout the development of TalkAssist inspired us, challenged us, and enabled us to transform our vision into a working product.
...
\noindent\textbf{Setup:} The tester shut their eyes or used a pillowcase to simulate blindness. The online version of TalkAssist ran on a Windows laptop in a quiet room. Only voice commands were used to navigate between features such as time queries, reminders, and basic Q\&A responses.

\noindent\textbf{Environment \& Tools:}
\begin{itemize}
...
\noindent\textbf{Purpose:} To validate whether TalkAssist meets its accessibility goal: functioning as a screen-reader-free, voice-only assistant.
...
\noindent\textbf{Setup:} Several recording environments were tested:
...
\noindent Additionally, multiple microphones were used (laptop mic vs. USB headset).

\noindent\textbf{Purpose:} To determine if TalkAssist maintains voice recognition accuracy across common, real-world usage environments, as many visually impaired users rely on built-in laptop hardware that varies widely in quality.
...
\noindent\textbf{Setup:} A series of spoken math prompts were used, including:
...
\noindent\textbf{Purpose:} To validate the correctness of the math parser and ensure disambiguation logic prevents false triggers.

\subsubsection{Experiment 4: Offline Speech Recognition \& Reminder Management}
...
\noindent\textbf{Setup:}
\begin{itemize}
    \item Hardware: Windows 11 laptop, USB microphone
    \item Software: Whisper base model (70M parameters) running locally, Python 3.10 virtual environment
    \item Environments:
    \begin{itemize}
        \item Quiet room
...
\noindent Offline module tests included:
...
\noindent\textbf{Purpose:} To ensure the assistant remains usable in scenarios with poor connectivity: highly relevant to low-vision users in unpredictable environments.
...
\noindent\textbf{Setup:} The tester interacted with TalkAssist using standard voice commands and manually observed the single available frontend indicator:

\begin{itemize}
    \item The recording indicator, which is designed to appear only when user audio input is being captured.
\end{itemize}

\noindent\textbf{Purpose:} To determine whether the recording indicator switches states properly during typical voice interactions and whether it provides reliable feedback to users who depend on nonvisual cues.
...
\subsubsection*{Dataset for Experiment 4 (Offline Mode)}
...
\subsubsection*{Dataset for Experiment 5 (GUI Test)}
...
\subsubsection{Experiment 1 Results: Online Voice-Only Navigation}
...
\noindent\textbf{Failure point:} accessing the reminders page required too strict phrasing and occasionally misrouted the user.

\noindent This indicates a need for broader intent recognition or additional aliases for “open reminders.”

\subsubsection{Experiment 2 Results: Voice Recognition Across Environments}
...
\noindent This matches expected patterns in consumer STT systems: microphone quality is the primary determinant of accuracy.

\subsubsection{Experiment 3 Results: Mathematical Query Parsing}
...
\noindent worked as intended, preventing false time-query activations.

\noindent This confirms the math parser’s dependability and robustness to ambiguous phrasing.

\subsubsection{Experiment 4 Results: Offline Whisper STT \& Reminder System}
...
\noindent Whisper performed slightly worse than online STT but remained usable for reminders and date/time queries. Whisper’s deterministic behavior significantly improved reliability for structured tasks such as reminders.
...
\noindent Offline mode proved highly stable because it avoids ambiguity and latency associated with large online models.

\subsubsection{Experiment 5 Results: GUI Indicator Test}

\noindent\textbf{GUI Indicator Behavior}
...
\noindent Analysis: The issue stems from the online SpeechRecognition API continuously streaming even during bot speech. Because TalkAssist is designed to operate in a fully voice-driven mode, the system does not explicitly pause speech recognition during text-to-speech playback. As a result, the frontend remains in a listening state, causing the recording indicator to remain active incorrectly.

\noindent A potential solution would be to temporarily disable or pause speech recognition while the assistant is speaking, then re-enable it once speech output completes. Alternatively, frontend event handling could be restructured to gate recording state changes based on confirmed turn completion rather than continuous audio input. These approaches would improve clarity and usability for visually impaired users who rely on accurate feedback cues.

\subsubsection{Expected and Unexpected Findings}
...
\noindent As a result, offline mode became the most stable subsystem, which is valuable for visually impaired users who may not always have reliable connectivity.

\subsubsection{Failure Cases \& Limitations}
...
\noindent These findings will guide future improvements, including better frontend event handling, expanded intent recognition, and optimization of the offline pipeline.
...
\noindent\textbf{Reminder Data Storage:}
\begin{itemize}
...
\noindent\textbf{Privacy and Data Protection:}
...
\noindent\textbf{Online Service Compliance:}
...
\noindent\textbf{Third-Party API Privacy Concerns (e.g., ElevenLabs):}
\begin{itemize}
...
\noindent\textbf{Privacy \& User Consent:}
\begin{itemize}
...
\noindent\textbf{Inclusivity and Accessibility:}
...
\noindent The development of TalkAssist resulted in an accessible voice driven assistant that supports both online and offline functionality for visually impaired users. Through this project, we learned how to integrate speech technologies, manage system modes, and design interactions that minimize dependence on visual interfaces.

\noindent Our implementation shows that offline mode currently performs more reliably than online mode for core tasks such as retrieving the time and date and managing reminders. Because all the processings are handled locally, offline mode offers faster responses and remains functional even without internet connectivity, an area where many existing assistants struggle.

\noindent Online mode, while enabling access to external services, introduces challenges such as network dependency, latency, and synchronization issues when switching between modes. Future work will focus on improving online reliability through better state management, smoother mode transitions, and enhanced error handling. Additional improvements may include expanded offline language understanding, integration of online services, and more advanced wake word detection to further enhance accessibility and user independence.
...
\bibitem{skypage}
Skypage. \textit{The Evolution of AI Voice Assistants: Usage Patterns and Adoption Trends in North America}. 2025.  
https://skywork.ai/skypage/en/The-Evolution-of-AI-Voice-Assistants:-Usage-Patterns-and-Adoption-Trends-in-North-America/1950063575851700224

\bibitem{un}
United Nations. \textit{Building an Accessible Future for All: AI and the Inclusion of Persons with Disabilities}. Feb. 14, 2024.  
https://unric.org/en/building-an-accessible-future-for-all-ai-and-the-inclusion-of-persons-with-disabilities/

\bibitem{alexa}
Amazon. \textit{Amazon Alexa Accessibility Features}.  
https://www.amazon.com/gp/help/customer/display.html?nodeId=G200127470

\bibitem{nvda}
NV Access. \textit{About NVDA}. September 9, 2025.  
https://www.nvaccess.org/about-nvda/

\bibitem{seeingai}
Lewis, V. \textit{How I Use Microsoft Seeing AI with Low Vision}. Perkins, July 2025.  
https://www.perkins.org/resource/microsoft-seeing-ai-and-low-vision-review/

\bibitem{elevenlabs}
ElevenLabs. \textit{Introducing ElevenLabs Agents}. 2025.  
https://elevenlabs.io/blog/introducing-elevenlabs-agents

\bibitem{ablr}
Angeles, Maria. \textit{The Pros and Pitfalls of AI for People Who Are Blind or Low Vision}.  
Ablr, June 23, 2025.  
https://ablr360.com/the-pros-and-pitfalls-of-ai-for-people-who-are-blind-or-low-vision/

\bibitem{techrxiv}
TechRxiv. \textit{Analysis of AI-Driven Digital Accessibility for the Visually Impaired Through Non-Visual Modalities}. 2025.  
https://www.techrxiv.org/users/839525/articles/1237247-analysis-of-ai-driven-digital-accessibility-for-the-visually-impaired-through-non-visual-modalities-challenges-and-prospects

\bibitem{qualimero}
Qualimero. \textit{Importance, Challenges, and AI Solutions of Digital Accessibility}. 2025.  
https://www.qualimero.com/en/blog/digital-accessibility-ai-revolution

\bibitem{cacm}
Communications of the ACM. \textit{AI and Accessibility}. June 1, 2020.  
